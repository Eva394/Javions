...
class CprDecoderTest {
    ...
    void checkDecodePosition(int cprX0, int cprY0, int cprX1, int cprY1, int mostRecent, double expectedLonDeg,
                             double expectedLatDeg, double delta) {
        var x0 = cpr( cprX0 );
        var x1 = cpr( cprX1 );
        var y0 = cpr( cprY0 );
        var y1 = cpr( cprY1 );
    ...
    void cprDecoderDecodePositionWorksOnKnownExamples() {
        ...
        checkDecodePosition( 0b01100100010101100, 0b10110101101001000, 0b01100010000010010, 0b10010000110101110, 0,
                             3.919373, 52.257202, delta );
    ...
    void cprDecoderDecodePositionReturnsNullWhenSwitchingLatitudeBands() {
        var args = new int[][]{
                // Random values
                {43253, 99779, 122033, 118260}, {67454, 100681, 123802, 124315}, {129578, 70001, 82905, 105074},
                {30966, 110907, 122716, 79872},
                // Real values
                {85707, 77459, 81435, 60931}, {100762, 106328, 98304, 89265}, {104941, 106331, 104905, 89210},};
