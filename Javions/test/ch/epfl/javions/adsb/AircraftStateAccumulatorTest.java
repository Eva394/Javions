...
class AircraftStateAccumulatorTest {
    ...
    void aircraftStateAccumulatorUpdateUpdatesCategoryAndCallSign() {
        ...
        for ( var i = 0 ; i < 8 ; i += 1 ) {
            ...
            var message = new AircraftIdentificationMessage( expectedLastMessageTimeStampNs, icao, expectedCategory,
                                                             expectedCallSign );
    ...
    void aircraftStateAccumulatorUpdateUpdatesVelocityAndTrackOrHeading() {
        ...
        for ( var i = 0 ; i < 8 ; i += 1 ) {
            ...
            var message = new AirborneVelocityMessage( expectedLastMessageTimeStampNs, icao, expectedVelocity,
                                                       expectedTrackOrHeading );
            accumulator.update( message );
        }
    }
    ...
    void aircraftStateAccumulatorUpdateUpdatesAltitudeButNotPositionWhenParityIsConstant() {
        ...
        for ( int parity = 0 ; parity <= 1 ; parity += 1 ) {
            ...
            for ( int i = 0 ; i < 100 ; i += 1 ) {
                ...
                var x = 0.999999 / ( i + 1d );
                var y = 1d - x;
                var message = new AirbornePositionMessage( expectedLastMessageTimeStampNs, icao, expectedAltitude,
                                                           parity, x, y );
                accumulator.update( message );
            }
        }
    ...
    void aircraftStateAccumulatorUpdateUpdatesAltitudeButNotPositionWhenMessagesTooFarApart() {
        ...
        for ( int i = 0 ; i < 100 ; i += 1 ) {
            ...
            parity = 1 - parity;
            var message = new AirbornePositionMessage( expectedLastMessageTimeStampNs, icao, expectedAltitude, parity,
                                                       x, y );
            accumulator.update( message );
        }
    }
    ...
    void aircraftStateAccumulatorCorrectlyHandlesLatitudeBandChange() {
        ...
        var xys = new ParityXY[]{new ParityXY( 0, 98152, 106326 ), new ParityXY( 1, 95758, 89262 ),
                                 new ParityXY( 0, 95758, 106330 ), new ParityXY( 1, 93364, 89266 )};
        var expectedLongitudeDeg = 6.57520;
        var expectedLatitudesDeg = new double[]{Double.NaN, 46.8672, 46.8672, 46.8674};
